...
# It is a simple fit to the Mauna Loa data, accounting for the seasonal
# variability and the yearly increase.  In tests, the error was within about 4 ppm
################################################################################
...
def find_cloud(irsch1, vceil, vip, verbose):
    ...
    vflag = np.zeros(len(irsch1['secs']), dtype=int)         # The cloud flag
        ...
        # Only TROPoe uses the logig to deal with the pseudo-ob, so set a flag accordingly
    try:
        tmp = vip['cbh_pobs_flag']
        use_pobs = 1
    except:
        use_pobs = 0
    ...
    if use_pobs == 1:
        ...
        pobmn = np.ones(len(irsch1['secs']))*(-999.)
        pobsg = np.ones(len(irsch1['secs']))*(-999.)
